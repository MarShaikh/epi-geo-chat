...
def traced_agent(span_name: str, capture_args: List[str] | None = None, capture_output: str | None = None):
    def decorator(func):
        @wraps(func)
        async def async_wrapper(*args, **kwargs):
            tracer = trace.get_tracer(__name__)
                ...
                if capture_args:
                    for arg_name in capture_args:
                        if arg_name in kwargs:
                            ...
                            span.set_attribute(f"agent.arg.{arg_name}", _serialize_args(value))
                
                result = await func(*args, **kwargs)
                
                if capture_output:
                    span.set_attribute(f"agent.output.{capture_output}", _serialize_args(result))
